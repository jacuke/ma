Dieser Abschnitt behandelt die versionsübergreifende Suche nach Umsteigern.
...
\struktkommentar{Die Schleife wird solange durchlaufen bis es keine weitere Version in der vorgegebenen Suchrichtung gibt. Pro Durchlauf wird die Variable \texttt{\$merge} wie oben beschrieben befüllt. Abhängig davon, ob die anonyme Funktion \texttt{\$function} definiert ist, wird diese entweder ausgeführt {\color{blue} §TODO: Verweis auf ConceptMap-Schreiben?} oder die Variable \texttt{\$data} für die Rückgabe befüllt.}
...
\struktogrammAO{90}{
    ...
    \ifend
    \while[\heightNS]{FOREACH \$entry IN readData(\$system, \$version, '', 'umsteiger')}
        \assign[\heightNS]{\$currentCode = \$entry[ \$current ]}
        \assign[\heightNS]{\$otherCode = \$entry[ \$other ]}
...
Der erste Eintrag wird übernommen, der zweite kommt neu hinzu und für den dritten Eintrag wird G83.88 $\Rightarrow$ G83.8 entsprechend umgewandelt, weil es G83.8 schon gibt.
