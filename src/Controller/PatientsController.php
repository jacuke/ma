...
class PatientsController extends AbstractController  {
    ...
    public function patients(Request $request): Response  {
        ...
        $total_count = $this->patientsRepo->countPatients();
        $search_count = 0;
        $max_page = 0;

        if(!$init) {
            $search_count = $this->patientsRepo->countPatients($search_code, $search_name);
            $max_page = floor($search_count/PatientsRepository::PAGE_SIZE);
            if($search_count % PatientsRepository::PAGE_SIZE !==0) {
                $max_page++;
            }
            $patients = $this->patientsRepo->readPatients($search_code, $search_name, $page);
