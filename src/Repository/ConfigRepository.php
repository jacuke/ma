...
class ConfigRepository extends DatabaseRepository {
    ...
    public function readConfigStatus(string $key, bool $check_if_table_exists = false) : string {

        if($check_if_table_exists) {
            if(!$this->tableExists(Constants::TABLE_CONFIG)) {
                return Constants::CONFIG_STATUS_NO_TABLE;
            }
        }
